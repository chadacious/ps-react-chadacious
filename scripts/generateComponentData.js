...
function generate(paths) {
    ...
    var componentData = getDirectories(paths.components).map(function(componentName) {
        try {
            ...
            errors.push('An error occirred while attempting to generate metadata for ' + componentName + '. ' + error);
        }
    });
...
function getExampleFiles(examplesPath, componentName) {
    ...
    try {
        ...
        console.log(chalk.red(`No examples found for ${componentName}.`) + error);
    }
    return exampleFiles;
}
